...
//     }
//     const hashedPassword = await bcrypt.hash(password, 10);
//     const user = new User({ email, hashedPassword, name, role });
//     await user.save();
//     const token = jwt.sign({ id: user._id.toString(), role: user.role, name: user.name }, process.env.JWT_SECRET, { expiresIn: "1h" });
//     res.status(201).json({ message: "User registered successfully", token, userId: user._id.toString(), name: user.name, role: user.role });
//   } catch (error) {
